...
std::atomic<uint32_t> s_otaActive{0};
std::atomic<uint32_t> s_otaSuccessTotal{0};
std::atomic<uint32_t> s_otaFailuresTotal{0};
...
void metrics_set_ota_active(bool active) {
  s_otaActive.store(active ? 1U : 0U, std::memory_order_relaxed);
}

void metrics_record_ota_success() {
  s_otaSuccessTotal.fetch_add(1, std::memory_order_relaxed);
}

void metrics_record_ota_failure() {
  s_otaFailuresTotal.fetch_add(1, std::memory_order_relaxed);
}
...
String metrics_render_prometheus() {
  ...
  append_uint_metric(out, "candle_ota_active", s_otaActive.load(std::memory_order_relaxed));

  append_meta(out, "candle_ota_success_total", "Successful OTA uploads.", "counter");
  append_uint_metric(out, "candle_ota_success_total", s_otaSuccessTotal.load(std::memory_order_relaxed));
  append_meta(out, "candle_ota_failures_total", "Failed OTA uploads.", "counter");
  append_uint_metric(out, "candle_ota_failures_total", s_otaFailuresTotal.load(std::memory_order_relaxed));
  ...
  out += escape_prometheus_label(String(cfg.devname[0] != '\0' ? cfg.devname : "candle-light"));
  out += "\",firmware_version=\"";
  out += escape_prometheus_label(String(FIRMWARE_VERSION));
  out += "\",build_commit=\"";
  out += escape_prometheus_label(String(BUILD_GIT_COMMIT));
  out += "\",build_date=\"";
  out += escape_prometheus_label(String(BUILD_DATE));
  out += "\",ip=\"";
